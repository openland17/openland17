# Matt Smith

Independent AI consultant | Brisbane, Australia.
I build production ML and automation systems for businesses that don't have an in-house ML team. Profile pic is the office I work out of. Come stop by and say hi!

```
work     →  mjs-consulting.com.au
models   →  huggingface.co/Mattysmittttt
contact  →  matt@mjs-consulting.com.au
```

---

### Currently

- Building **CamoNet** | fine-grained military camouflage classifier, DINOv2 backbone, 40 patterns. [→ model](https://huggingface.co/Mattysmittttt/camonet)
- Shipping client work in email-to-event automation, donor intelligence and hospitality analytics.

### Stack I reach for

Python · PyTorch · HuggingFace (Transformers / Datasets / Hub) · FastAPI · Next.js · Postgres · Supabase · Modal / Lambda Labs for training

### Selected work

| | |
|---|---|
| **CamoNet** | Fine-grained camouflage pattern classifier. DINOv2-base, 40 classes, full pipeline open-source. |
| **Bowser** | Fuel price comparison app for AU, route-based AI cost-benefit engine across 6,000+ stations. [bowserapp.com.au](https://bowserapp.com.au) |
| **Donor Intelligence** | Donor analytics dashboard for a Foundation. Closed-source. |
| **MJS Consulting** | AI automation consulting practice. Aus SMB focus. [mjs-consulting.com.au](https://mjs-consulting.com.au) |

### Reading list / influences

Sutton & Barto · Bishop's PRML · Karpathy's lectures · *The Hundred-Page ML Book* · Simon Willison's blog.

---

<sub>The best way to reach me about consulting work is email. Open to interesting collaboration on applied vision and agent systems.</sub>
